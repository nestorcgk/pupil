...
class Frame(object):
    def __init__(self,img,timebase):
        self.img = img
        self.gray = cv2.cvtColor(img, cv.CV_BGR2GRAY)
...
class Camera_Capture(object):
    ...
    def __init__(self,uid,timebase=None):
        ...
        self.capture = cv2.VideoCapture(uid)       
        self.uid = uid
    ...
    def re_init(self,uid,size=(640,480),fps=30):
        ...
        self.use_hw_ts = False
        self.capture = cv2.VideoCapture(uid)
        self.capture.set(cv2.CV_CAP_PROP_FRAME_WIDTH, size[0])
        self.capture.set(cv2.CV_CAP_PROP_FRAME_HEIGHT, size[1])
        self.uid = uid
